   ...
   */
      ...
      for(std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j = m_socketAddresses.begin(); j != m_socketAddresses.end (); ++j)
        {
	      ...
	      //NS_LOG_DEBUG ("First: " << beaconPos.first);
	      //NS_LOG_DEBUG ("Second: " << beaconPos.second);
   ...
   point trilateration(point point1, point point2, point point3, double r1, double r2, double r3) {
    ...
    RoutingProtocol::UpdateHopsTo (Ipv4Address beacon, uint16_t newHops, double x, double y)
    {
      ...
      NS_LOG_DEBUG("oldHops: " << oldHops);
      NS_LOG_DEBUG("newHops: " << newHops);
     
     point finalPose;
     srand(1546564564);
     double pos = 100.0 + (fRand(0, 4) - 2);
     double d1,d2,d3 = 0.0;
     point p1 = {123.42, 4535.452};
     point p2 = {6663.42, 566.646};
     point p3 = {123.42, 9873.45};

     for(int i = 0; i < 10; i++)
     {
	d1 = sqrt((pow(123.42-pos, 2)) + (pow(4535.452-pos,2)));
	d2 = sqrt((pow(6663.42-pos, 2)) + (pow(566.646-pos, 2)));
	d3 = sqrt((pow(123.42-pos, 2)) + (pow(9873.45-pos, 2)));

	finalPose = trilateration(p1,p2,p3,d1,d2,d3);

	NS_LOG_DEBUG("Trilateration X Coord for Node " << i << " : " << finalPose.x);
	NS_LOG_DEBUG("Trilateration Y Coord for Node " << i << " : " << finalPose.y);

	pos = pos + 100 + (fRand(0, 4) - 2);
     }
